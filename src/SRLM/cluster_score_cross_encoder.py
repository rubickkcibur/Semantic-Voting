...
def compute_model_based_sim(texts):
    device = "cuda:0"
    model = transformers.AutoModelForSequenceClassification.from_pretrained('cross-encoder/ms-marco-TinyBERT-L2-v2').to(device)
    tokenizer = transformers.AutoTokenizer.from_pretrained('cross-encoder/ms-marco-TinyBERT-L2-v2')
    total_num = len(texts)
    pairs = []
    ...
    for i in range(0, len(pairs), 64):
        ...
        features = tokenizer(batch_pairs, padding=True, truncation=True, return_tensors="pt").to(device)
        model.eval()
        with torch.no_grad():
            batch_scores = model(**features).logits
            scores.extend(batch_scores[:, 0].detach().cpu().numpy().tolist())
    max_v = max(scores)
    min_v = min(scores)
    scores = [(s - min_v) / (max_v - min_v) if (max_v - min_v) > 0 else 0 for s in scores]
    sim_matrix = np.zeros((total_num, total_num))
    for i in range(total_num):
        sim_matrix[i, i] = 1.0
        for j in range(i):
            sim_matrix[i, j] = scores.pop(0)
...
def compute_cluster_scores(sentence_pairs, args):
    with tqdm.tqdm(total=len(sentence_pairs), desc="Compute Cluster Scores") as pbar:
        for item in sentence_pairs:
            ...
            cands = item["extracted_candidates"]
            valid_inputs = [cand for cand in cands if is_valid_answer(cand)]
            sim_matrix = compute_model_based_sim(valid_inputs)
            ...
            cos_sim_cluster = []
            for idx in main_cluster:
                total_sim = sum([sim_matrix[idx, jdx] for jdx in main_cluster])/len(main_cluster)
                cos_sim_cluster.append(total_sim)
