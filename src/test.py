...
# with jsonlines.open("/mnt/maclabcv2/rubickjiang/codes/open-r1/data/SR_candidates/dpo_data_cos_score.jsonl", "r") as fr:
#     sentence_pairs = [item for item in fr]
# def extract_pred(txt):
#     pattern = r'\\boxed\{([^}]*)\}'
#     results = re.findall(pattern, txt)
#     if results:
...
# chosen_score = 0
# reject_score = 0
# dataset = load_dataset("/mnt/maclabcv2/rubickjiang/public_dataset/wmt24pp", "en-zh_CN")
# dataset["train"] = dataset["train"].filter(lambda example: not example["is_bad_source"])
# def find_ref(prompt):
#     for item in dataset["train"]:
#         if item["target"].strip() == prompt:
#             return item["source"]
#     return None

# for item in sentence_pairs:
#     prompt = item["prompt"].split("The Chinese sentence is: '")[-1].split("'\nPlease think about how to translate step by step.")[0].strip()
#     ref = find_ref(prompt)
#     chosen = extract_pred(item["chosen"])
#     chosen = "None" if chosen is None else chosen
#     rejected = extract_pred(item["rejected"])
#     rejected = "None" if rejected is None else rejected
    # chosen = "source: " + prompt + " candidate: " + chosen + " reference: " + ref 
    # rejected = "source: " + prompt + " candidate: " + rejected + " reference: " + ref
    # encodings = tokenizer(
    #     [chosen, rejected],
    #     max_length=512,
    #     truncation=True,
    ...
    #     scores = outputs.predictions.detach().cpu().float().numpy().tolist()
    #     item["chosen_score"] = scores[0]
    #     item["reject_score"] = scores[1]
    #     chosen_score += scores[0]
    #     reject_score += scores[1]
# print("Chosen Score:", chosen_score / len(sentence_pairs))
# print("Reject Score:", reject_score / len(sentence_pairs))
# with jsonlines.open("/mnt/maclabcv2/rubickjiang/codes/open-r1/data/SR_candidates/dpo_data_cos_score_debug.jsonl", "w") as fw:
#     for item in sentence_pairs:
#         fw.write(item)
# quit()
