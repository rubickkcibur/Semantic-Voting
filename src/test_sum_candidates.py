...
bleurt_model = BleurtForSequenceClassification.from_pretrained('/mnt/maclabcv2/rubickjiang/proj_storage/huggingface_models/BLEURT-20')
bleurt_tokenizer = BleurtTokenizer.from_pretrained('/mnt/maclabcv2/rubickjiang/proj_storage/huggingface_models/BLEURT-20')
bleurt_model.to("cuda:0")
bleurt_model.eval()
def reward_model_score(preds, refs):
    pred_txt = [pred if pred is not None else "None" for pred in preds]
    encodings = bleurt_tokenizer(refs, pred_txt, return_tensors='pt', padding="longest").to("cuda:0")
    with torch.no_grad():
        scores = bleurt_model(**encodings).logits.flatten().detach().cpu().float().numpy().tolist()
    return scores

def compute_metrix(path, gts):
    data = []
    ...
    for item in tqdm.tqdm(data):
        ...
        bleurt_score = reward_model_score(preds, [item["gt"]] * len(preds))
        for result, score in zip(total_results, bleurt_score):
            result.update({"BLEURT": score})
        item.update({"scores": total_results})
    return data
...
def test_designed_metric(scored_path):
    device = "cuda:1"
    tokenizer = transformers.AutoTokenizer.from_pretrained(
        "/mnt/maclabcv2/rubickjiang/proj_storage/huggingface_models/unsup-simcse-bert-base-uncased",
        padding_side="left",
    ...
    with jsonlines.open(scored_path, "r") as fr:
        data = [item for item in fr]
    model.eval()
...
os.environ["MACLAB_NAS_NAME"] = MACLAB_NAS_NAME
...
gts = [ item["highlights"] for item in dataset["train"] ]

# scored_data = compute_metrix("/mnt/maclabcv2/rubickjiang/codes/open-r1/data/SR_candidates/Qwen2.5-1.5B-Instruct/cnn_dailymail_output_64.jsonl", gts)
# with jsonlines.open("/mnt/maclabcv2/rubickjiang/codes/open-r1/data/SR_candidates/Qwen2.5-1.5B-Instruct/cnn_dailymail_output_debug.jsonl", "w") as fw:
#     for item in scored_data:
#         fw.write({
#             "prompt": item["prompt"],
#             "candidates": item["candidates"],
#             "scores": [score["BLEURT"] for score in item["scores"]]
#         })
test_designed_metric("/mnt/maclabcv2/rubickjiang/codes/open-r1/data/SR_candidates/Qwen2.5-1.5B-Instruct/cnn_dailymail_output_debug.jsonl")