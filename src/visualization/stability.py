...
self = [
    ...
    [2.06, 10.00, 2.81, 11.01, 1.82, 10.77, 2.57, 11.86, 16.53, 38.02, 28.27, 52.54],
...
entropy = [
    ...
    [1.51, 10.52, 4.35, 10.37, 4.66, 8.47, 5.21, 10.56, 14.45, 25.66, 28.14, 45.69],
...
for i in range(len(base)):
    for j in range(len(base[0])):
        if base[i][j] is None:
            ...
            self[i][j] = (map_MQM(self[i][j]) - map_MQM(base[i][j]))/base[i][j] * 100
            entropy[i][j] = (map_MQM(entropy[i][j]) - map_MQM(base[i][j]))/base[i][j] * 100
            cluster[i][j] = (map_MQM(cluster[i][j]) - map_MQM(base[i][j]))/base[i][j] * 100
        else:
...
g = sns.relplot(x="lexical", y="semantic", hue="Method", size="size", 
            ...
            height=6, data=dataset, aspect=1.3)
...
g.set_axis_labels("Lexical metric (%)", "Semantic metric (%)", size=16)
plt.setp(g.legend.get_texts(), fontsize=14)
plt.savefig("stability_plot.png", dpi=300)